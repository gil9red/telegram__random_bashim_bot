...
class Quote(BaseModel):
    ...
    def get_all_with_comics(cls, where: ModelSelect = None) -> ModelSelect:
        ...
        return (
            query.join(Comics, JOIN.LEFT_OUTER)
            .group_by(cls)
...
if __name__ == "__main__":
    ...
    assert admin.find_quote_ids("Arux") == admin.find_quote_ids(
        "Arux", case_insensitive=False
    )
    ...
    # Quotes with comics
    print(
        "Total quotes having comics of admin:", admin.get_total_quotes(with_comics=True)
    )
    ...
    sub_query = Request.get_all_quote_id_by_user(
        admin, fields=[Request.quote_id, Request.date_time]
    )
    quote_id = 102776
    items = [
        (i, get_date_str(x.date_time))
        for i, x in enumerate(sub_query)
        if x.quote_id == quote_id
    ]
    ...
    print(f"Quote #{quote_id} found in:\n{text}")
    print()
